...
p5.prototype.createSVG = function(...args) {
  return new p5.SVG(...args);
};

p5.prototype.drawSVG = function(svg, x = 0, y = 0) {
  svg.draw(this, x, y);
};

p5.prototype.loadSVG = function(path, successCallback, failureCallback) {
  const svg = new p5.SVG();
  const self = this;
  ...
  fetch(path, req)
    ...
    .then(text => {
      let dom = parseXml(text);
      let json = xml2json(dom);
      //console.log(json);
      svg.jsonParse(JSON.parse(json));
